...
def update():
    """从 github 获取最新接口"""
    apiList = ['https://hk1.monika.love/OpenEthan/SMSBoom/master/','Online Download Page']
    apiS = int(input("选择更新的apiS接口[0.自带  1.SuperLinkStudioCloud]:"))
    if apiS == 1:
        GETAPI_json_url = 'https://file.weycloud.online/p/Public_Files/SMS-boom/GETAPI.json?sign=lSeNeHETFjFK0ty0lPsGenhZ5csmDDcXwvqwhXT3ihk=:0&alist_ts=1712390744275'
        API_json_url = "https://file.weycloud.online/p/Public_Files/SMS-boom/api.json?sign=_w7l1JaFhIDO1Ip-2TeWdtX9nekgkZSPQbbs8LQyzPQ=:0&alist_ts=1712390945021"
    else:
    ...
    try:
        with httpx.Client(verify=False, timeout=10) as client:
            GETAPI_json = requests.get(GETAPI_json_url).text
            api_json = requests.get(API_json_url).text
